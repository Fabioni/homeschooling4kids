													...
													while ( $loop->have_posts() ) : $loop->the_post();
														...
														<div class="horizontal-scroll-item">
															<article
																...
																<div
																	class="post-item <?= ( strtotime( get_the_time( "d.m.Y H:i:s" ) ) > time() ) ? "deaktiviert" : "" ?> <?= get_the_time( 'Ymd' ) === current_time( 'Ymd' ) ? "heutigerBeitrag" : "" ?>">
																	<?php
																	$korrek = "";
																	if ( isset( $_GET['korrektur'] ) && $_GET['korrektur'] == "true" ) {
							...
							<div class="timeline-content-wrapper">
								...
								<a class="bnt-more" href="<? /*= get_term_link($fa) */ ?>">Mehr</a>-->
									<div class="timeline-img-header"
										 style="background: linear-gradient(rgba(0, 0, 0, 0), rgba(0, 0, 0, .4)), url('<?= get_field( "themabild", $fa ) ?>') center center/cover no-repeat;">
										<h2><?= $fa->name ?></h2>
									</div>
									<div
										class="date"><?= ( $fa->startdate === null ) ? "geplant" : ( "ab " . date( "d.m.Y", strtotime( $fa->startdate ) ) ) //TODO Monat als österreischiches Wort ?></div>
									<p><?= $fa->description ?></p>
									<p style="font-size: 80%"><?= implode( " ... ", array_map( function ( $postid ) {
											$p = get_post( $postid );

											return $p->post_title;
										}, explode( ",", $fa->postids ) ) ) ?></p>
